...
class Checkout extends React.Component {
   ...
   */
  order = async () => {
    let total = this.cartRef.current.calculateTotal();
    if (this.state.balance < total) {
      message.error('Wallet Balance is less than the total ammount, please reduce some item')
    }
    else if (this.state.addresses.length == 0) {
      message.error('Please provide the address for delivery');
    }
    else {
      this.checkout();
    }
  ...
  async componentDidMount() {
    if (localStorage.getItem('username') && localStorage.getItem('token')) {
      ...
      message.error('Please login into your account first');
                            ...
                            <div className="address">
                              <Radio.Button value={index}>
                                <div className="address-box">
                                  ...
                                  <Button
                                    type="primary"
                                    onClick={async () => { await this.deleteAddress(address._id) }}
                                  >
                                    Delete
                                  </Button>
                                </div>
